  ...
  <section class="work" id="js-work">
    ...
    <ul class="work__list">
      <li
        ...
        <a :href="`projects/${item.slug}`" class="work__link" @mouseover="(event) => _mouseOverHandler(event, index)" @mouseleave="_mouseLeaveHandler">
  ...
  methods: {
    ...
    _setUpTimelines () {
      ...
      for (let i = 0; i < this.$refs.title.length; i++) {
        ...
        if (i + 1 === this.$refs.title.length) {
          tl.to(this.$refs.lastLine, { duration: 0.9, scaleX: 1 }, 0.9)
        }
      }
    },
